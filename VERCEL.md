...
当前 workflow 会优先检查正式的 `VERCEL_TOKEN`；如果 token 已失效，会自动尝试 `VERCEL_AUTH_JSON_B64` 备份登录态。两者都不可用时，部署 job 会明确失败并提示刷新 GitHub Secrets。本机 Vercel CLI 的短期 token 不能直接当 CI 的 `--token` 使用，会出现 `The token provided via --token argument is not valid`。后面如果在 Vercel Account Tokens 页面创建了正式 Access Token，可以更新 `VERCEL_TOKEN` 并删除 `VERCEL_AUTH_JSON_B64`。
